...
export const handleView = async ({
    ...
    if (encodeUrl) {
        const base64 = fromByteArray(encode({ contract, method, args, actions: { field, redirect } }));
        const encodedUrl = url.origin + '/v1/e/' + base64
        return new Response(JSON.stringify({ encodedUrl }), { headers: jsonHeaders })
    }

    // not a bot
    if (redirect && botMap && !/facebookexternalhit|Discordbot|Twitterbot|LinkedInBot|TelegramBot/gi.test(userAgent)) {
